function Invoke-ROCheckinSecret {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    try {
        ...
        $response = Invoke-ROApi -Session $Session -Endpoint "secrets?filter.isCheckedOut=true&take=20"
        ...
        foreach ($rec in $response.records) {
            $detail = Invoke-ROApi -Session $Session -Endpoint "secrets/$($rec.id)"
        ...
        Invoke-ROApi -Session $Session -Endpoint "secrets/$($secret.id)/check-in" -Method POST -Body $body | Out-Null
