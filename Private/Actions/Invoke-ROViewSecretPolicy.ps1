function Invoke-ROViewSecretPolicy {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    try {
        $policies = Invoke-ROApi -Session $Session -Endpoint "secret-policy/search?take=20"
        ...
        if ($policies.records -and $policies.records.Count -gt 0) {
            ...
            # View detail
            Invoke-ROApi -Session $Session -Endpoint "secret-policy/search?filter.secretPolicyName=$($policy.secretPolicyName)" | Out-Null
        }
