function Invoke-ROTriggerHeartbeat {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    try {
        ...
        $response = Invoke-ROApi -Session $Session -Endpoint "secrets?filter.heartbeatStatus=Failed&take=20"
        if (-not $response.records -or $response.records.Count -eq 0) {
            # Fall back to success status if no failed ones
            $response = Invoke-ROApi -Session $Session -Endpoint "secrets?filter.heartbeatStatus=Success&take=20"
        }
        if (-not $response.records -or $response.records.Count -eq 0) {
