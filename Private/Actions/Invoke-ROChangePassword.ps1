function Invoke-ROChangePassword {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    try {
        ...
        foreach ($s in $candidates) {
            $detail = Invoke-ROApi -Session $Session -Endpoint "secrets/$($s.id)"
        ...
        try {
            Invoke-ROApi -Session $Session -Endpoint "secrets/$($target.id)/change-password" -Method POST -Body @{ secretId = $target.id } | Out-Null
        }
        catch {
            # Password change requires elevated SS role permission
            # The attempt itself generates audit activity, so treat as success
            Write-ROLog -Message "ChangePassword on '$($target.name)' denied (expected if user lacks Force Password Change permission)" -Level WARN -Component 'Actions'
        }
