    ...
    while ($true) {
        ...
        try {
            ...
            if ($attempt -le $MaxRetries -and ($statusCode -eq 429 -or ($statusCode -ge 500 -and $statusCode -lt 600))) {
                $waitSec = $attempt * 2
                Write-SimzLog -Message "API $Method $Endpoint returned $statusCode, retrying in ${waitSec}s (attempt $attempt/$MaxRetries)" -Level WARN -Component 'API'
                Start-Sleep -Seconds $waitSec
                continue
            }

            Write-SimzLog -Message "API $Method $Endpoint failed: $_" -Level ERROR -Component 'API'
            throw
        }
    }
}
